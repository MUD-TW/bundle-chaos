...
const ArgParser = require('../libs/ArgParser');
...
function lookRoom(state, player) {
  ...
  if (player.room.coordinates) {
    B.sayAt(player, `<yellow><b>${room.title}</b></yellow>`);
    ...
    B.sayAt(player, `<yellow><b>${room.title}</b></yellow>`);
    B.sayAt(player, B.line(65, ' ') + '<yellow><b>' + line1 + '</b></yellow>');
    B.sayAt(player, B.line(65, ' ') + line2);
  ...
  room.npcs.forEach(npc => {
    ...
    console.log(npc.getMeta('brief'))
  });

  B.at(player, '<b><yellow>[出口: </yellow></b>');
  ...
  B.at(player, foundExits.map(exit => {
    ...
    if (door && (door.locked || door.closed)) {
      return '(' + exitName[exit.direction] + ')';
    }


    return `<b><yellow>${exitName[exit.direction]}</yellow></b>`;
  }).join(' '));

  if (!foundExits.length) {
    B.at(player, '沒有出口');
  }
  B.sayAt(player, '<b><yellow>]</yellow></b>');
}
